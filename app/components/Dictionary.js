...
export default function Dictionary({ generals, tactics, activeSynergies, selectedGenerals, selectedTactics }) {
  ...
  const [dictSubTab, setDictSubTab] = useState('generals'); // 'generals' | 'tactics' | 'synergies'
    ...
    <>
      <div>
        ...
        <div className="classic-subtab-bar">
          <button
            onClick={() => setDictSubTab('generals')}
            className={`classic-subtab ${dictSubTab === 'generals' ? 'active' : ''}`}
          >
            장수도감
          </button>
          <button
            onClick={() => setDictSubTab('tactics')}
            className={`classic-subtab ${dictSubTab === 'tactics' ? 'active' : ''}`}
          >
            전법도감
          </button>
          <button
            onClick={() => setDictSubTab('synergies')}
            className={`classic-subtab ${dictSubTab === 'synergies' ? 'active' : ''}`}
          >
            인연도감
          </button>
        </div>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                      {findRecommendedGenerals(detailTactic, generals).length > 0 ? (
                        findRecommendedGenerals(detailTactic, generals).map(g => (
                          <span key={g.id} className="recommend-tag">
                            {g.name}
                          </span>
                        ))
                      ) : (
        ...
        {dictSubTab === 'synergies' && (
          <section className="scroll-panel">
            {(() => {
              ...
              // 결성된 인연을 먼저, 그 안에서는 매칭된 인원 수 많은 순으로 정렬
                ...
                <>
                  ...
                  <div className="synergy-grid">
                    {synergyStatus.map(synergy => (
                      ...
                      >
                        <h3 style={{
                          margin: '0 0 6px 0', fontSize: '1.2rem',
