      ...
      {general.recommended_tactics && general.recommended_tactics.length > 0 && (
        <div style={{ 
          ...
          <ul style={{ margin: 0, paddingLeft: '16px', lineHeight: '1.4' }}>
            {general.recommended_tactics.slice(0, 3).map((tactic, idx) => (
              <li key={idx} style={{ marginBottom: '2px' }}>{tactic}</li>
            ))}
            {general.recommended_tactics.length > 3 && (
              <li style={{ color: isSelected ? '#ccc' : '#777' }}>+ {general.recommended_tactics.length - 3}개 더</li>
            )}
          </ul>
        </div>
      )}
